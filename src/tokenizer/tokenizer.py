...
def tokenize_data(input_data, token_path):
    """
    Tokenizes the input data based on the provided token mappings.

    Args:
    input_data (str): The raw input data as a string.
    token_path (str): File path to the JSON file containing token mappings.
...
def tokenize_data_multiprocessing(input_data, token_path, out_path, batch_size=100000):
    """
...
def tokenize_file(
    token_path, data_path, out_path, multiprocessing=True, batch_size=100000
):
    """
    The main function that reads input data, tokenizes it, and writes the tokenized data to an output file.

    Args:
    - token_path (str): Path to the JSON file containing token mappings.
    ...
    if multiprocessing:
        tokenize_data_multiprocessing(input_data, token_path, out_path, batch_size)
    else:
        tokenized_data = tokenize_data(input_data, token_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "token_path", help="Path to the JSON file containing token mappings."
    )
    parser.add_argument(
        "data_path",
    ...
    tokenize_file(args.token_path, args.data_path, args.out_path)
