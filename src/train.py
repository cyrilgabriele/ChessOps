...
class ChessTrainer:
    """
    This class handles the training of a GPT-2 model on the Chess dataset.
    ...
    - learning_rate (float): The learning rate for training the GPT-2 model.
    ...
    class ChessDataset(Dataset):
        ...
        def __init__(
            self, games: list[list[int]], max_length: int, padding_id: int
        ) -> None:
            self.games = games
        ...
        def __getitem__(self, index: int) -> torch.Tensor:
            padded_game = [self.padding_id] * (
                self.max_length - len(self.games[index])
            ) + self.games[index]
    ...
    def create_dataset(self) -> ChessDataset:
        ...
        return self.ChessDataset(games, max_length, self.PAD_TOKEN_ID)
    ...
    def train(self) -> None:
        """
        Trains the GPT-2 model on the Chess dataset.
        """
