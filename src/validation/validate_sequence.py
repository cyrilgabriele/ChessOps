...
def is_valid_move_pawn(move, color):
    """
    ...
    multiplier = 1 if color else -1  # True means it's white's turn
...
def is_valid_move_king(move):
    ...
    is_castling_move = move in CASTLING_MOVES
...
def get_error_category(
    ...
    if board.outcome() or number_of_plies_until_error == len(game_moves):
        return "no error"
        ...
        elif is_piece_logic_error(error_move, color, piece_type):
            return "piece logic"

        elif is_path_obstruction_error(error_move, color, piece_type, board):
...
def is_piece_logic_error(move, color, piece_type):
    """
    ...
    if piece_type == chess.PAWN:
        if not is_valid_move_pawn(move, color):
            return True
...
def evaluate_sequence(game_sequences, token_sequences, debug=False, notation="xLAN"):
    ...
    Error types:
        *No Error:* The move is legal, but the game ended or reached the max sequence length.
...
def validate_sequence(
    ...
    Args:
        ...
        token_path (str): Path to the JSON file containing the tokens.
